...
#define MAX_PRIM_NAME_LEN 16

struct Name {
	const int  len;
	const char name[MAX_PRIM_NAME_LEN];
};

const struct Name prim_names[]={
{7, "(docol)"          },
{7, "(docon)"          },
{7, "(dovar)"          },
{8, "(douser)"         },
...
static inline void printinst(struct cost *c)
{
  static char *states="1023456789";
	...
	*out = '_';
	...
	PrimNum*       prims = super2 + c->offset;
	PrimNum* const end   = prims  + c->length;

	do {
		struct Name const* name = prim_names + *prims;
...
//  struct waypoint* wp = inst[ninsts];
  ...
  for (i=ninsts-1; i>=0; i--) {
    ...
    for (j=1; j<=MAX_SUPER && i+j<=ninsts; j++) {
      struct SuperState const *superp = lookup_super(origs+i, j);
      if ( superp!=NULL ) {
